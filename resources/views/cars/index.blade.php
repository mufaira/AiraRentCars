            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex items-center justify-between h-20">
                    ...
                    <nav class="hidden md:flex items-center gap-8">
                        ...
                        <a href="{{ route('cars.catalog') }}" class="text-sm font-medium text-[#f53003]">Katalog</a>
        ...
        <main class="pt-32 pb-24">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="bg-black dark:bg-[#1a1a1a] rounded-2xl border border-white/10 p-8 mb-12">
                    ...
                    <form action="{{ route('cars.catalog') }}" method="GET" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                        <div>
                            ...
                            <select name="status" class="w-full bg-[#1a1a1a] border border-white/20 text-white rounded-lg px-4 py-2.5 hover:border-white/40 focus:border-[#f53003] transition">
                                ...
                                <option value="Tersedia" {{ request('status') === 'Tersedia' ? 'selected' : '' }} class="bg-black">Tersedia</option>
                                <option value="Disewa" {{ request('status') === 'Disewa' ? 'selected' : '' }} class="bg-black">Disewa</option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="type" class="w-full bg-[#1a1a1a] border border-white/20 text-white rounded-lg px-4 py-2.5 hover:border-white/40 focus:border-[#f53003] transition">
                                ...
                                <option value="Manual" {{ request('type') === 'Manual' ? 'selected' : '' }} class="bg-black">Manual</option>
                                <option value="Matic" {{ request('type') === 'Matic' ? 'selected' : '' }} class="bg-black">Matic</option>
                            </select>
                        </div>
                        ...
                        <div class="flex gap-3 items-end">
                            ...
                            @if (request()->filled('status') || request()->filled('type') || request()->filled('min_price') || request()->filled('max_price'))
                                <a href="{{ route('cars.catalog') }}" class="flex-1 bg-gray-700 hover:bg-gray-600 text-white font-bold px-4 py-2.5 rounded-lg transition text-center">
                                    Reset
                                </a>
                            @endif
                        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 mb-12">
                    @forelse ($cars as $car)
                        <div class="group relative bg-[#1a1a1a] rounded-2xl overflow-hidden hover:shadow-2xl hover:shadow-orange-500/10 transition-all duration-300 hover:-translate-y-2 border border-white/5 dark:hover:border-[#f53003]/30">
                            <div class="relative overflow-hidden bg-gray-800 h-48">
                                @if ($car->featured_photo)
                                    <img src="{{ asset('storage/' . $car->featured_photo->photo_path) }}" 
                                         alt="{{ $car->name }}" class="w-full h-full object-cover transform group-hover:scale-110 transition duration-700">
                                @else
                                ...
                                <div class="absolute top-4 right-4">
                                    <span class="px-3 py-1 rounded-lg text-xs font-bold {{ $car->status === 'Tersedia' ? 'bg-green-500/80 text-white' : 'bg-red-500/80 text-white' }} backdrop-blur-sm">
                                        {{ $car->status }}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div class="p-6">
                                <h3 class="text-lg font-bold text-white mb-3">{{ $car->name }}</h3>
                                ...
                                <div class="flex items-center justify-between pt-4 border-t border-white/10">
                                    ...
                                    <a href="{{ route('cars.detail', $car) }}" class="p-3 bg-[#f53003] hover:bg-[#d63000] text-white rounded-xl transition shadow-lg transform hover:-translate-y-0.5">
