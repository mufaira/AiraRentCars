...
<div class="min-h-screen bg-black text-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <!-- Back Button -->
        <a href="{{ route('cars.catalog') }}" class="inline-flex items-center gap-2 text-gray-400 hover:text-[#f53003] transition mb-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        @foreach ($car->photos as $photo)
                            <button onclick="document.getElementById('mainPhoto').src = this.querySelector('img').src" 
                                ...
                                <img src="{{ asset('storage/' . $photo->photo_path) }}" 
                                     alt="{{ $car->name }}" class="w-full h-full object-cover group-hover:scale-110 transition duration-300">
                            </button>
                        @endforeach
                    </div>
                @endif
            ...
            <div class="lg:col-span-1">
                <div class="bg-gradient-to-br from-[#1a1a1a] via-[#1a1a1a] to-[#0a0a0a] border border-white/10 rounded-2xl p-8">
                    <!-- Title -->
                    <h1 class="text-3xl font-bold text-white mb-4">{{ $car->name }}</h1>
                    ...
                    <div class="mb-6">
                        <span class="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold
                            ...
                            <span class="w-2 h-2 rounded-full {{ $car->status === 'Tersedia' ? 'bg-green-400' : 'bg-red-400' }}"></span>
                            {{ $car->status }}
                        </span>
                    </div>
                    ...
                    <div class="space-y-4 mb-6 pb-6 border-b border-white/10">
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <span class="font-semibold text-white">{{ $car->created_at->format('d M Y') }}</span>
                        </div>
                    </div>
                    ...
                    @if ($car->description)
                        <div class="mb-6 pb-6 border-b border-white/10">
                            ...
                            <p class="text-gray-400 text-sm leading-relaxed">{{ $car->description }}</p>
                        </div>
                    @endif
                    ...
                    <div class="space-y-3">
                        @auth
                            ...
                            @elseif ($car->status === 'Tersedia')
                                <a href="{{ route('rentals.create', $car) }}" class="w-full inline-flex items-center justify-center gap-2 bg-gradient-to-r from-[#f53003] to-orange-600 hover:from-[#d63000] hover:to-orange-700 text-white px-4 py-3 rounded-lg font-bold transition duration-200">
                        ...
                        @else
                            @if ($car->status === 'Tersedia')
                                <a href="{{ route('login') }}" class="w-full inline-flex items-center justify-center gap-2 bg-gradient-to-r from-[#f53003] to-orange-600 hover:from-[#d63000] hover:to-orange-700 text-white px-4 py-3 rounded-lg font-bold transition duration-200">
                        ...
                        <a href="https://wa.me/62812345678?text=Halo%2C%20saya%20tertarik%20dengan%20{{ urlencode($car->name) }}%20di%20DriveHub.%20Bisakah%20Anda%20memberikan%20informasi%20lebih%20lanjut%3F" target="_blank" rel="noopener noreferrer" class="w-full border border-[#f53003]/50 text-[#f53003] hover:bg-[#f53003]/10 px-4 py-3 rounded-lg font-semibold transition duration-200 inline-flex items-center justify-center gap-2">
            ...
            <div class="mt-16">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    @foreach ($relatedCars as $relatedCar)
                        <a href="{{ route('cars.detail', $relatedCar) }}" class="group relative overflow-hidden bg-gradient-to-br from-[#1a1a1a] via-[#1a1a1a] to-[#0a0a0a] border border-white/10 hover:border-[#f53003] rounded-2xl transition duration-300">
                            <!-- Image -->
                            <div class="relative h-48 overflow-hidden bg-gradient-to-br from-gray-800 to-gray-900">
                                @if ($relatedCar->featured_photo)
                                    <img src="{{ asset('storage/' . $relatedCar->featured_photo->photo_path) }}" 
                                         alt="{{ $relatedCar->name }}" class="w-full h-full object-cover group-hover:scale-110 transition duration-500">
                                @endif
                                
                                <!-- Status Badge -->
                                <div class="absolute top-4 right-4">
                                    <span class="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold {{ $relatedCar->status === 'Tersedia' ? 'bg-green-500/20 text-green-400 border border-green-500/50' : 'bg-red-500/20 text-red-400 border border-red-500/50' }}">
                                        <span class="w-2 h-2 rounded-full {{ $relatedCar->status === 'Tersedia' ? 'bg-green-400' : 'bg-red-400' }}"></span>
                                        {{ $relatedCar->status }}
                                    </span>
                                </div>
                            </div>
                            ...
                            <!-- Content -->
                            <div class="p-4">
                                <h3 class="text-lg font-bold text-white mb-2 group-hover:text-[#f53003] transition">{{ $relatedCar->name }}</h3>
                                <div class="flex items-center gap-2 text-gray-400 text-sm mb-3">
                                    <span>{{ $relatedCar->transmission }}</span>
                                    <span>•</span>
                                    <span>{{ $relatedCar->capacity }} orang</span>
                                </div>
                                <p class="text-[#f53003] font-bold text-lg">Rp {{ number_format($relatedCar->price_per_day, 0, ',', '.') }}/hari</p>
                            </div>
                        </a>
                    @endforeach
                </div>
