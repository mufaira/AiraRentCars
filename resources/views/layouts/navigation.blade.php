<nav x-data="{ open: false }" class="fixed top-0 z-50 w-full bg-black/95 backdrop-blur-md border-b border-white/10 transition-all duration-300">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-center items-center h-20 relative">
            ...
            @auth
                @if(Auth::user()->is_admin)
                    <a href="{{ route('admin.dashboard') }}" class="absolute left-0 flex items-center gap-2 group shrink-0">
                @else
                    <a href="{{ route('dashboard') }}" class="absolute left-0 flex items-center gap-2 group shrink-0">
                @endif
            @else
                <a href="{{ route('dashboard') }}" class="absolute left-0 flex items-center gap-2 group shrink-0">
            @endauth
            ...
            <div class="absolute right-0 flex items-center gap-4">
                @auth
                    @if(!request()->routeIs('admin.dashboard'))
                        @if(Auth::user()->is_admin || Auth::user()->is_staff)
                            <a href="{{ route('cars.admin.index') }}" class="hidden lg:inline-flex text-sm font-medium text-gray-400 hover:text-white transition">
                                Kelola
                            </a>
                        @endif
                        @if(Auth::user()->is_admin)
                            <a href="{{ route('admin.dashboard') }}" class="hidden lg:inline-flex text-sm font-medium text-gray-400 hover:text-white transition">
                                Admin
                            </a>
                        @endif
                    @endif
                ...
                @else
                    ...
                    @if (Route::has('register'))
                        <a href="{{ route('register') }}" class="px-5 py-2.5 text-sm font-bold text-white bg-[#f53003] rounded-full hover:bg-[#d63000] hover:shadow-lg hover:shadow-orange-500/30 transition transform hover:-translate-y-0.5">
                            Daftar
                        </a>
                    @endif
                @endauth
        ...
        <div :class="open ? 'block' : 'hidden'" class="md:hidden pb-6 space-y-2">
            <a href="/" class="block px-4 py-2 text-gray-400 hover:text-white hover:bg-white/5 rounded-lg transition {{ request()->url() === url('/') ? 'text-white bg-white/10' : '' }}">
                Beranda
            </a>
            <a href="{{ route('cars.catalog') }}" class="block px-4 py-2 text-gray-400 hover:text-white hover:bg-white/5 rounded-lg transition {{ request()->routeIs('cars.catalog', 'cars.detail') ? 'text-white bg-white/10' : '' }}">
                Katalog
            </a>
            @auth
                @if(Auth::user()->is_admin)
                    <a href="{{ route('admin.dashboard') }}" class="block px-4 py-2 text-gray-400 hover:text-white hover:bg-white/5 rounded-lg transition {{ request()->routeIs('admin.dashboard') ? 'text-white bg-white/10' : '' }}">
                        Panel Admin
                    </a>
                @else
                    <a href="{{ route('rentals.index') }}" class="block px-4 py-2 text-gray-400 hover:text-white hover:bg-white/5 rounded-lg transition {{ request()->routeIs('rentals.index', 'rentals.show') ? 'text-white bg-white/10' : '' }}">
                        Sewa Saya
                    </a>
                @endif
            @endauth
            <a href="{{ route('blogs.index') }}" class="block px-4 py-2 text-gray-400 hover:text-white hover:bg-white/5 rounded-lg transition {{ request()->routeIs('blogs.index', 'blogs.show') ? 'text-white bg-white/10' : '' }}">
                Blog
            </a>
            @auth
                @if(Auth::user()->is_admin || Auth::user()->is_staff)
                    <a href="{{ route('cars.admin.index') }}" class="block px-4 py-2 text-gray-400 hover:text-white hover:bg-white/5 rounded-lg transition">
                        Kelola Mobil
                    </a>
                @endif
            @endauth
