<nav x-data="{ open: false }" class="fixed top-0 z-50 w-full bg-black/95 backdrop-blur-md border-b border-white/10 transition-all duration-300">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-center items-center h-20 relative">
            ...
            <?php if(auth()->guard()->check()): ?>
                <?php if(Auth::user()->is_admin): ?>
                    <a href="<?php echo e(route('admin.dashboard')); ?>" class="absolute left-0 flex items-center gap-2 group shrink-0">
                <?php else: ?>
                    <a href="<?php echo e(route('dashboard')); ?>" class="absolute left-0 flex items-center gap-2 group shrink-0">
                <?php endif; ?>
            <?php else: ?>
                <a href="<?php echo e(route('dashboard')); ?>" class="absolute left-0 flex items-center gap-2 group shrink-0">
            <?php endif; ?>
                ...
                <span class="text-xl font-extrabold tracking-tight hidden sm:inline">
            ...
            <!-- Right Side: Dashboard Button & User Menu - Positioned Absolutely on Right -->
            <div class="absolute right-0 flex items-center gap-4">
                <?php if(auth()->guard()->check()): ?>
                    <?php if(!request()->routeIs('admin.dashboard')): ?>
                        <?php if(Auth::user()->is_admin || Auth::user()->is_staff): ?>
                            <a href="<?php echo e(route('cars.admin.index')); ?>" class="hidden lg:inline-flex text-sm font-medium text-gray-400 hover:text-white transition">
                                Kelola
                            </a>
                        <?php endif; ?>
                        <?php if(Auth::user()->is_admin): ?>
                            <a href="<?php echo e(route('admin.dashboard')); ?>" class="hidden lg:inline-flex text-sm font-medium text-gray-400 hover:text-white transition">
                                Admin
                            </a>
                        <?php endif; ?>
                    <?php endif; ?>
                    ...
                    <div x-data="{ dropdownOpen: false }" class="relative">
                        <button @click="dropdownOpen = !dropdownOpen" @click.away="dropdownOpen = false" class="flex items-center gap-2 px-4 py-2.5 rounded-full text-gray-300 hover:text-white hover:bg-white/5 transition">
                            <div class="w-8 h-8 bg-gradient-to-br from-[#f53003] to-orange-600 rounded-full flex items-center justify-center">
                                <svg class="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clip-rule="evenodd"></path>
                                </svg>
                            </div>
                            <span class="text-sm font-semibold hidden sm:inline"><?php echo e(Auth::user()->name); ?></span>
                            <svg :class="dropdownOpen ? 'rotate-180' : ''" class="w-4 h-4 transition" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <?php else: ?>
                    ...
                    <?php if(Route::has('register')): ?>
                        <a href="<?php echo e(route('register')); ?>" class="px-5 py-2.5 text-sm font-bold text-white bg-[#f53003] rounded-full hover:bg-[#d63000] hover:shadow-lg hover:shadow-orange-500/30 transition transform hover:-translate-y-0.5">
                            Daftar
                        </a>
                    <?php endif; ?>
                <?php endif; ?>
            ...
            <div class="md:hidden absolute right-0">
                <button @click="open = !open" class="p-2 rounded-lg text-gray-400 hover:text-white hover:bg-white/5 transition">
        ...
        <div :class="open ? 'block' : 'hidden'" class="md:hidden pb-6 space-y-2">
            <a href="/" class="block px-4 py-2 text-gray-400 hover:text-white hover:bg-white/5 rounded-lg transition <?php echo e(request()->url() === url('/') ? 'text-white bg-white/10' : ''); ?>">
                Beranda
            </a>
            <a href="<?php echo e(route('cars.catalog')); ?>" class="block px-4 py-2 text-gray-400 hover:text-white hover:bg-white/5 rounded-lg transition <?php echo e(request()->routeIs('cars.catalog', 'cars.detail') ? 'text-white bg-white/10' : ''); ?>">
                Katalog
            </a>
            <?php if(auth()->guard()->check()): ?>
                <?php if(Auth::user()->is_admin): ?>
                    <a href="<?php echo e(route('admin.dashboard')); ?>" class="block px-4 py-2 text-gray-400 hover:text-white hover:bg-white/5 rounded-lg transition <?php echo e(request()->routeIs('admin.dashboard') ? 'text-white bg-white/10' : ''); ?>">
                        Panel Admin
                    </a>
                <?php else: ?>
                    <a href="<?php echo e(route('rentals.index')); ?>" class="block px-4 py-2 text-gray-400 hover:text-white hover:bg-white/5 rounded-lg transition <?php echo e(request()->routeIs('rentals.index', 'rentals.show') ? 'text-white bg-white/10' : ''); ?>">
                        Sewa Saya
                    </a>
                <?php endif; ?>
            <?php endif; ?>
            <a href="<?php echo e(route('blogs.index')); ?>" class="block px-4 py-2 text-gray-400 hover:text-white hover:bg-white/5 rounded-lg transition <?php echo e(request()->routeIs('blogs.index', 'blogs.show') ? 'text-white bg-white/10' : ''); ?>">
                Blog
            </a>
            <?php if(auth()->guard()->check()): ?>
                <?php if(Auth::user()->is_admin || Auth::user()->is_staff): ?>
                    <a href="<?php echo e(route('cars.admin.index')); ?>" class="block px-4 py-2 text-gray-400 hover:text-white hover:bg-white/5 rounded-lg transition">
                        Kelola Mobil
                    </a>
                <?php endif; ?>
            <?php endif; ?>
